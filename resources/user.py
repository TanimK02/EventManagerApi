...
class UserRegister(MethodView):
    ...
    def post(self, user_info):
        ...
        if db.session.execute(db.select(UserModel).where(UserModel.username == user_info["username"])).scalar():
            abort(409, message="username already exists")
        if db.session.execute(db.select(UserModel).where(UserModel.email == user_info["email"])).scalar():
        ...
        result = db.session.execute(db.select(UserModel).where(UserModel.username == user_info["username"])).scalar_one()
    ...
    def post(self):
        ...
        try:
            if user_info.get("username", None):
                user = db.session.execute(db.select(UserModel).where(UserModel.username == user_info["username"])).scalar_one()
            elif user_info.get("email", None):
                user = db.session.execute(db.select(UserModel).where(UserModel.email == user_info["email"])).scalar_one()
            else:
                abort(400, message="Need username or email")
        except SQLAlchemyError:
            abort(404, message="User does not exist")
        if bcrypt.checkpw(user_info["password"].encode('utf-8'), user.password):
            ad_claims = {"Model": "User"}
